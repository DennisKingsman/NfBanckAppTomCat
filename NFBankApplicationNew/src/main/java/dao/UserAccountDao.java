...
public class UserAccountDao {
    ...
    private static final String SQL_REQUEST_INSERT_INIT = "insert into user_account(user_login, user_password, user_role) " +
                                                            "values (?, ?, ?) returning user_id";
    private static final String SQL_REQUEST_SELECT_PROFILE = "select * from user_account where user_login = ?";
    private static final String SQL_REQUEST_LOGIN = "select user_id, user_login, user_password from user_account where user_login = ? and user_password = ?";
    ...
    public UserAccount loginPerson(String login, String password){
        ...
        try (Connection connection = getConnection();
            ...
            if(resultSet.next()){
                ...
                Long userIdFromBD = resultSet.getLong("user_id");
                logger.info("ALERT!!! user_id: " + userIdFromBD);
                userAccount.setId(userIdFromBD);
    ...
    public Long registerUserAccount(String login, String password){
        ...
        try (Connection connection = getConnection();
            ...
            Long res = 0L;
            if(resultSet.next()){
                res = resultSet.getLong("user_id");
            }

            logger.info("register complete. RES: " + res);
            return res;
